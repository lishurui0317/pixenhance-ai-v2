...
class OnnxUpscaler:
    ...
    def run(self, src: Path, dst: Path) -> Tuple[bool, str]:
        ...
        if out.ndim != 4 or out.shape[0] != 1 or out.shape[1] != 3:
            return False, f"ONNX 输出 shape 异常：{out.shape}"
...
def _run_subprocess(cmd: list[str], cwd: Optional[str]) -> Tuple[bool, str]:
    ...
    try:
        result = subprocess.run(
            ...
            timeout=600, creationflags=flags, cwd=cwd,
        )
...
def _tiled_vulkan_upscale(
    ...
    # 步长 = tile_px - overlap（输出尺寸是输入 x4，所以输入步长 = 输出步长 / 4）
    step_in = (tile_px - overlap) // 4
    if step_in < 1:
        step_in = 1
    for ty in range(0, sh, step_in):
        for tx in range(0, sw, step_in):
            y0 = ty
            x0 = tx
            ...
            try:
                ...
                # 重叠区在输出尺寸 = overlap * 4
                pad_out = overlap * 4
                lx0 = 0 if x0 == 0 else pad_out
                ly0 = 0 if y0 == 0 else pad_out
                lx1 = ow if x1 == sw else ow - pad_out
                ly1 = oh if y1 == sh else oh - pad_out
                up_crop = up.crop((lx0, ly0, lx1, ly1))
                # 写到 canvas 的位置
                pos_x = x0 * 4 + (pad_out if x0 != 0 else 0)
                pos_y = y0 * 4 + (pad_out if y0 != 0 else 0)
                canvas.paste(up_crop, (pos_x, pos_y))
            finally:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                ...
                for future in as_completed(future_map):
                    ...
                    if ok:
                        success_count += 1
                        self.log(
                            f"[{idx}/{total}] ✓ {src.name} → {dst.name}  ({engine_used})",
                            "success",
                        )
                    else:
                        fail_count += 1
...
class ImageUpscalerApp(_BaseTk):
    ...
    def _pick_best_model(files: List[Path]) -> str:
        ...
        启发式：
          ...
          - 路径含 "anima" / "6B" / "illustration" → realesrgan-x4plus-anime-6B
          - 路径含 "x2" / "small" → realesrgan-x2plus
        ...
        if any(k in joined for k in ("anime", "动漫", "二次元", "cartoon", "漫画")):
            if any(k in joined for k in ("6b", "anima", "illustration", "插画")):
                return "realesrgan-x4plus-anime-6B"
            return "realesrgan-x4plus-anime"
        if any(k in joined for k in ("x2", "small", "tiny")):
